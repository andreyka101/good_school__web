...
const phone = document.querySelectorAll(".link_phone") as NodeListOf<HTMLLinkElement>
// const email = document.querySelector("#email_footer") as HTMLLinkElement
const email = document.querySelectorAll(".link_email") as NodeListOf<HTMLLinkElement>
console.log(1);
console.log(phone);
    ...
    el.addEventListener("click", () => {
        if (detect.mobile() == null) {
            ...
            navigator.clipboard.writeText("+79189700187").then(function () {
                ...
                el.classList.add("button_active_1")
                el.innerHTML = `<div class="svg"></div>
                <div class="information_text">
                    номер скопирован
                </div>`
                setTimeout(() => {
                    el.classList.remove("button_active_1")
                    el.innerHTML = `<div class="svg"></div>
            <div class="information_text">
                +79189700187
            </div>`
    ...
    el.addEventListener("click", () => {
        if (detect.mobile() == null) {
            navigator.clipboard.writeText("goodschool80@mail.ru").then(function () {
                console.log('Текст успешно скопирован в буфер обмена');
                el.classList.add("button_active_2")
                el.innerHTML = `<div class="svg"></div>
            <div class="information_text">
            почта скопирована
            </div>`
                setTimeout(() => {
                    el.classList.remove("button_active_2")
                    el.innerHTML = `
                    <div class="svg"></div>
                    <div class="information_text">
